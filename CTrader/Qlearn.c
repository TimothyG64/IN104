...
envOutput Q_step(action a){
    int done = 0;
    envOutput stepOut;
    //condition de non sortie du terrain
    if(faisabilite(s,a) == true){
        if (a==up && state_row > 1){
        state_row = max(0,state_row -1);
        }else if (a==down && state_row < rows -2){
            state_row = min(rows,state_row +1);
        }else if (a==right && state_col < cols -2){
            state_col = min(cols,state_col +1);
        }else if (a==upup && state_row < rows -3){                         //AJOUTS
            state_row = max(0,state_row -2); 
        }else if (a==downdown && state_row > 2){
            state_row = max(0,state_row +2);
        }
    }
    
    printf("%d %d %d %s \n", state_row,state_col, a, faisabilite ? "true" : "false");
...
void init_RewardQ(void)
{
        ...
        for (i = 0; i < rows; ++i) {
                for (j = 0; j < cols; ++j) {
                        if (maze[i][j] == '+') {
                                RewardTab[i][j] = 10;   //c'est un mur
                        } else if (maze[i][j] == 'g') {
                                RewardTab[i][j] = 10;   
                        } else {
                                RewardTab[i][j] =- 40;  //on sort du chemin
                        }
                }
        }
}
...
void SARSA(float gamma, float alpha){
    ...
    for(int i=0;i<I_max;i++){
        ...
        while(st.done!=1 ){
            ...
            // si on ne sort pas du terrain
            if(faisabilite(st,at1) == true){
                st=st1; //on avance
                at =at1;
            }
            k++;
